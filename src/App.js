...
const HOTMART  = "https://hotmart.com/produto/jarbis-pro";
...
async function dbGet(table, col, val) {
  try {
    const r = await fetch(`${SUPA_URL}/rest/v1/${table}?${col}=eq.${encodeURIComponent(val)}&select=*`, { headers: H });
    return await r.json();
  } catch { return []; }
}
...
async function fazerCadastro(nome, email, senha) {
  const existe = await dbGet("usuarios", "email", email.toLowerCase().trim());
  if (existe && existe.length > 0) return { erro: "E-mail já cadastrado." };
  const u = await dbInsert("usuarios", { nome, email: email.toLowerCase().trim(), senha, plano: "gratis", assinatura_ativa: false });
  if (!u) return { erro: "Erro ao criar conta. Tente novamente." };
  return { usuario: u };
}

function loginGoogle() {
  const url = `${SUPA_URL}/auth/v1/authorize?provider=google&redirect_to=${encodeURIComponent(window.location.origin)}`;
...
const CIDADES = {
  ...
  "Guarulhos":    ["Centro","Taboão","Macedo","Vila Galvão","Jardim São João","Cumbica"],
...
const NOMES = {
  arquitetura:["Studio Arq","Forma Viva","Plano Criativo","Ápice Design","ArqMais","Vértice Studio","Geometria Arq","Linha Mestra","Conceito Arch","Prisma Arquitetura"],
  engenharia: ["Engenharia Sólida","Estrutura Pro","TechEng","Força Total Eng","Nexus Engenharia","Alpha Estruturas","Vertex Eng","Delta Técnica","Prime Eng"],
  construtora:["NovaBuild","CasaPrime","Constru Max","Edifica Brasil","MegaObras","Top Construções","Qualita Build","Urban Construtora","Alvo Obras"],
  imobiliaria:["Imóveis Prime","Casa & Cia","Grupo Imobiliário","Top Imóveis","Lar Perfeito","Alfa Imóveis"],
  industria:  ["Indústria Forte","Metal Tech","Ferro & Aço","Produção Total","IndusPark"],
  comercio:   ["Comércio Ativo","Supermercado União","Rede Atacado","Loja Central","Mercado Total"],
};

const MSG_PAD = `Olá! 😊\n\nMeu nome é *{NOME}*, sou eletricista profissional com *{ANOS} anos* de experiência.\n\nGostaria de me apresentar à *{EMPRESA}* como parceiro:\n\n⚡ Instalações elétricas completas\n⚡ Laudos e projetos (ART/RRT)\n⚡ Manutenção preventiva e corretiva\n⚡ SPDA e aterramento\n⚡ Adequação NR10 e NBR 5410\n\nPodemos conversar? 🤝`;
...
function gerarPDF(html, titulo) {
  try {
    ...
    w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${titulo}</title><style>body{font-family:Arial,sans-serif;font-size:13px;color:#111;padding:20px;max-width:700px;margin:0 auto;}table{width:100%;border-collapse:collapse;}th{background:#4f46e5;color:#fff;padding:8px;}td{padding:6px;border-bottom:1px solid #eee;}</style></head><body>${html}</body></html>`);
    w.document.close(); setTimeout(()=>{ w.focus(); w.print(); }, 500);
  } catch(e) { alert("Erro: "+e.message); }
}

function htmlDoc(os, emp) {
  const rows = (os.itens||[]).map(s=>`<tr><td>${s.n}</td><td align="center">${s.q}</td><td align="right">R$${fmtN(s.v)}</td><td align="right"><b>R$${fmtN(s.q*s.v)}</b></td></tr>`).join("");
  const total = (os.itens||[]).reduce((a,b)=>a+b.q*b.v,0)-(os.desconto||0);
  return `<div><div style="display:flex;justify-content:space-between;border-bottom:3px solid #4f46e5;padding-bottom:12px;margin-bottom:16px"><div><b style="font-size:18px;color:#4f46e5">${emp.nome||"Sua Empresa"}</b><br><small>${emp.cnpj||""} ${emp.crea?"CREA: "+emp.crea:""}</small><br><small>${emp.tel||""} ${emp.email||""}</small></div><div style="text-align:right"><b style="font-size:18px">${os.tipo==="contrato"?"CONTRATO":"OS / ORÇAMENTO"}</b><br><small>Nº ${os.numero||"001"} · ${os.data||hoje()}</small></div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px"><div style="background:#f5f5ff;padding:10px;border-radius:6px"><b>CLIENTE</b><br>${os.clienteNome||"—"}<br><small>${os.clienteTel||""}</small></div><div style="background:#f5f5ff;padding:10px;border-radius:6px"><b>Status:</b> ${STATUS_OS[os.status]?.l||""}<br><b>Local:</b> ${os.local||"—"}<br><b>Pagamento:</b> ${os.pagamento||"—"}</div></div>${os.descricao?`<div style="background:#fffbf0;border-left:4px solid #f59e0b;padding:8px 12px;margin-bottom:12px"><b>Descrição:</b> ${os.descricao}</div>`:""}<table><thead><tr><th align="left">Serviço / Material</th><th>Qtd</th><th>Unit.</th><th>Total</th></tr></thead><tbody>${rows}</tbody></table><div style="display:flex;justify-content:flex-end;margin:12px 0"><div style="min-width:180px">${os.desconto?`<div style="display:flex;justify-content:space-between;font-size:13px"><span>Desconto</span><span>-R$${fmtN(os.desconto)}</span></div>`:""}<div style="display:flex;justify-content:space-between;font-size:17px;font-weight:bold;border-top:2px solid #4f46e5;padding-top:6px"><span>TOTAL</span><span>R$${fmtN(total)}</span></div></div></div>${os.obs?`<div style="background:#f0f9ff;padding:10px;border-radius:6px;margin-bottom:12px"><b>Obs:</b> ${os.obs}</div>`:""}<div style="text-align:center;font-size:10px;color:#999;border-top:1px solid #eee;margin-top:20px;padding-top:8px">JARBIS PRO · ${hoje()}</div></div>`;
}
      ...
      {data.map((d,i)=>(
        <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
          <div style={{width:"100%",background:"rgba(255,255,255,.05)",borderRadius:3,height:64,display:"flex",alignItems:"flex-end",overflow:"hidden"}}>
            <div style={{width:"100%",background:color,borderRadius:3,height:Math.max(4,(d.v/max)*100)+"%",transition:"height .5s"}}/>
          </div>
...
.inp{width:100%;background:#1a2035;border:1.5px solid rgba(255,255,255,.08);color:#e2e8f0;border-radius:11px;padding:13px 15px;font-size:16px;outline:none;transition:border .2s;}
.inp:focus{border-color:#6366f1;}
.sel{width:100%;background:#1a2035;border:1.5px solid rgba(255,255,255,.08);color:#e2e8f0;border-radius:11px;padding:13px 15px;font-size:16px;outline:none;}
...
.btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;border:none;border-radius:11px;font-weight:700;transition:all .18s;font-size:15px;padding:13px 18px;}
...
.tb{display:flex;flex-direction:column;align-items:center;gap:2px;flex:1;min-width:54px;padding:10px 4px;border:none;background:none;color:#475569;border-bottom:3px solid transparent;transition:all .18s;}
...
.mbg{position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:900;display:flex;align-items:flex-end;justify-content:center;}
...
function TelaLogin({ onLogin }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0b0f1a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'DM Sans',sans-serif",color:"#e2e8f0"}}>
      <style>{CSS}</style>
      <div style={{textAlign:"center",marginBottom:28}}>
        <div style={{width:68,height:68,borderRadius:20,background:"linear-gradient(135deg,#6366f1,#818cf8)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 14px",boxShadow:"0 8px 32px rgba(99,102,241,.4)"}}>⚡</div>
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:30,fontWeight:800,background:"linear-gradient(135deg,#818cf8,#6366f1)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:2}}>JARBIS PRO</div>
        <div style={{fontSize:13,color:"#475569",marginTop:4}}>Sistema do Eletricista Profissional</div>
      </div>

      <div style={{width:"100%",maxWidth:400,background:"#111827",border:"1px solid rgba(255,255,255,.07)",borderRadius:20,padding:26}}>
        {/* Tabs */}
        <div style={{display:"flex",background:"#1a2035",borderRadius:12,padding:4,marginBottom:22,gap:4}}>
        ...
        <form onSubmit={modo==="login"?entrar:cadastrar} style={{display:"flex",flexDirection:"column",gap:14}}>
          {modo==="cadastro"&&(
            <div>
              ...
              <input className="inp" value={nome} onChange={e=>setNome(e.target.value)} placeholder="Ex: João Silva"/>
            </div>
          )}
          <div>
          ...
          <div>
            ...
            <input className="inp" type="password" value={senha} onChange={e=>setSenha(e.target.value)} placeholder="••••••••"/>
          </div>
          ...
          <button type="submit" className="btn btn-p" style={{width:"100%",fontSize:16,padding:"15px",marginTop:4}}>
            {load?<span style={{width:18,height:18,border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin 1s linear infinite",display:"inline-block"}}/>:null}
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,margin:"18px 0"}}>
          ...
          <span style={{fontSize:12,color:"#334155",fontWeight:600}}>ou</span>
          <div style={{flex:1,height:1,background:"rgba(255,255,255,.08)"}}/>
        </div>

        {/* Botão Google */}
        <button onClick={loginGoogle} style={{width:"100%",background:"#fff",border:"none",color:"#1a1a1a",borderRadius:12,padding:"14px",fontSize:15,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",gap:10,cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,.3)"}}>
        ...
        {modo==="cadastro"&&(
          <div style={{marginTop:14,background:"rgba(99,102,241,.06)",border:"1px solid rgba(99,102,241,.15)",borderRadius:12,padding:"12px 14px"}}>
            <div style={{fontSize:13,color:"#64748b",lineHeight:1.6}}>
              ✅ <b style={{color:"#e2e8f0"}}>Grátis:</b> Prospecção 5 empresas<br/>
              ⚡ <b style={{color:"#818cf8"}}>PRO R$47/mês:</b> Tudo ilimitado
            </div>
          </div>
        )}
      </div>
      <div style={{marginTop:14,fontSize:11,color:"#1e3a5f"}}>Ao continuar você concorda com os Termos de Uso</div>
    </div>
  );
}
    ...
    <div className="mbg" onClick={onClose}>
      <div className="mdl" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{textAlign:"center",marginBottom:18}}>
          ...
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:800,marginBottom:5,color:"#e2e8f0"}}>Recurso PRO</div>
          <div style={{fontSize:14,color:"#64748b"}}>Assine para desbloquear tudo</div>
        </div>
        {["✅ OS e Orçamentos ilimitados","✅ PDF profissional","✅ CRM completo","✅ Controle financeiro","✅ Contratos","✅ Prospecção ilimitada"].map(i=>(
          <div key={i} style={{fontSize:14,color:"#94a3b8",marginBottom:6}}>{i}</div>
        ))}
        <a href={HOTMART} target="_blank" rel="noreferrer" style={{display:"block",width:"100%",background:"linear-gradient(135deg,#6366f1,#818cf8)",color:"#fff",borderRadius:12,padding:"16px",fontSize:17,fontWeight:800,textAlign:"center",textDecoration:"none",marginTop:16,marginBottom:10}}>
          💳 Assinar por R$ 47/mês
        </a>
        <div style={{fontSize:12,color:"#475569",textAlign:"center",marginBottom:10}}>PIX · Cartão · Boleto</div>
...
export default function App() {
  // TODOS OS HOOKS PRIMEIRO — nunca depois de return condicional
  ...
  const [servicos,   setServicos]   = useState([
    {id:"p1",nome:"Ponto de tomada 20A",preco:85},
    {id:"p2",nome:"Ponto de iluminação",preco:70},
    {id:"p3",nome:"Ponto de chuveiro",preco:120},
    {id:"p4",nome:"Quadro de distribuição",preco:650},
    {id:"p5",nome:"Ar-condicionado split",preco:320},
    {id:"p6",nome:"Laudo elétrico + ART",preco:850},
    {id:"p7",nome:"SPDA / Para-raios",preco:1200},
    {id:"p8",nome:"Aterramento",preco:480},
  ]);
  ...
  const g6m   = Array.from({length:6},(_,i)=>{
    const m=(mesH-5+i+12)%12, a=m>mesH?anoH-1:anoH;
    return {l:MESES[m],v:financeiro.filter(f=>f.tipo==="receita"&&new Date(f.data).getMonth()===m&&new Date(f.data).getFullYear()===a).reduce((a,b)=>a+b.valor,0)};
  });

  // OS
  function abrirOS(tipo) {
    setEditOSId(null);
    setFormOS({numero:String(ordens.length+1).padStart(3,"0"),tipo:tipo||"orcamento",status:"orcamento",data:hoje(),pagamento:"À vista",clienteId:"",clienteNome:"",clienteTel:"",clienteEnd:"",local:"",descricao:"",obs:"",termos:"",desconto:0,sinal:0});
    setItensOS([]); setIN(""); setIV(""); setIQ("1"); setModalOS(true);
  }
  ...
  // Prospecção
  function gerarEmps() {
    const ns=[...(NOMES[pSeg]||[])].sort(()=>Math.random()-.5);
    return Array.from({length:Math.floor(Math.random()*6)+8},(_,i)=>({
      id:uid(), nome:ns[i%ns.length]+(Math.random()>.6?" "+pCidade.slice(0,3):""),
      segmento:pSeg, cidade:pCidade, bairro:pBairro,
      tel:"(11) "+["9","3","4"][~~(Math.random()*3)]+(~~(Math.random()*9000)+1000)+"-"+(~~(Math.random()*9000)+1000),
      porte:["Pequeno","Médio","Grande"][~~(Math.random()*3)], enviado:false
    }));
  }
  function iniciarScan() {
    ...
    pRef.current=setInterval(()=>{
      p+=Math.random()*4+2;
      if(p>=100){ clearInterval(pRef.current); p=100; let res=gerarEmps(); if(!isPro)res=res.slice(0,5); setPEmps(res); setPScan(false); setPDone(true); }
      setPPct(Math.min(100,p));
    },70);
  }
  function enviarWA(e) {
  ...
  // GUARDS — sempre após todos os hooks
    ...
    <div style={{minHeight:"100vh",background:"#0b0f1a",fontFamily:"'DM Sans',sans-serif",color:"#e2e8f0",overflowX:"hidden"}}>
      ...
      <div style={{background:"#111827",borderBottom:"1px solid rgba(255,255,255,.06)",padding:"12px 16px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:200}}>
        ...
        <div>
          ...
          <div style={{fontSize:11,color:"#475569"}}>Olá, {(usuario.nome||"Eletricista").split(" ")[0]}!</div>
        </div>
      ...
      <div className="tabs">
        {TABS.map(t=>(
          <button key={t.id} className={"tb"+(tab===t.id?" on":"")} onClick={()=>{
            ...
            <span className="lb">{t.lb}{["os","fin","cfg"].includes(t.id)&&!isPro?" 🔒":""}</span>
          </button>
        ))}
      </div>
      ...
      <div style={{maxWidth:620,margin:"0 auto",padding:"16px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {tab==="dash"&&<>
          ...
          <div className="card" style={{padding:16}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:14,fontWeight:700,marginBottom:10}}>Receita — 6 meses</div>
            <BarChart data={g6m}/>
          </div>
          <div className="card" style={{padding:16}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:14,fontWeight:700,marginBottom:11}}>Ações Rápidas</div>
            <button className="btn btn-p" onClick={()=>{if(!isPro){setShowUpgrade(true);return;}abrirOS("orcamento");}} style={{width:"100%",fontSize:16,padding:"15px",marginBottom:10}}>📋 Novo Orçamento / OS</button>
            <div className="g2">
              <button className="btn btn-g" onClick={()=>{if(!isPro){setShowUpgrade(true);return;}abrirOS("contrato");}} style={{fontSize:14,padding:"12px"}}>📄 Contrato</button>
              <button className="btn btn-o" onClick={()=>{if(!isPro){setShowUpgrade(true);return;}setModalFin(true);}} style={{fontSize:14,padding:"12px"}}>💰 Lançar</button>
            </div>
            <button className="btn btn-ghost" onClick={()=>setTab("prosp")} style={{width:"100%",fontSize:14,padding:"12px",marginTop:10}}>📡 Ir para Prospecção</button>
          </div>
            ...
            {ordens.slice(0,4).map(o=>{const st=STATUS_OS[o.status];return(
              <div key={o.id} onClick={()=>editOS(o)} style={{display:"flex",alignItems:"center",gap:11,padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,.05)",cursor:"pointer"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:14,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{o.clienteNome||"Sem cliente"}</div>
                  <div style={{fontSize:12,color:"#475569"}}>OS #{o.numero} · {o.data}</div>
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2}}>
                  <span style={{fontSize:14,fontWeight:700,color:"#4ade80"}}>{fmt(o.total)}</span>
        ...
        {tab==="prosp"&&<>
          ...
          <div className="card" style={{padding:16}}>
            ...
            {editMsg?(
              <div style={{display:"flex",flexDirection:"column",gap:9}}>
                <div style={{fontSize:12,color:"#475569"}}>Use: <code style={{color:"#818cf8"}}>{"{NOME}"}</code> · <code style={{color:"#818cf8"}}>{"{EMPRESA}"}</code> · <code style={{color:"#818cf8"}}>{"{ANOS}"}</code></div>
                <textarea value={msgP} onChange={e=>setMsgP(e.target.value)} rows={8} style={{width:"100%",background:"#1a2035",border:"1.5px solid rgba(99,102,241,.25)",color:"#e2e8f0",borderRadius:11,padding:"12px",fontSize:14,lineHeight:1.7,resize:"vertical"}}/>
                <button onClick={()=>setMsgP(MSG_PAD)} style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",color:"#64748b",borderRadius:9,padding:"8px",fontSize:12,fontWeight:600}}>↺ Restaurar padrão</button>
              </div>
            ):(
              <div style={{background:"#0d1525",borderRadius:10,padding:"10px 12px",maxHeight:80,overflow:"hidden",position:"relative"}}>
                ...
                <div style={{position:"absolute",bottom:0,left:0,right:0,height:24,background:"linear-gradient(transparent,#0d1525)"}}/>
              </div>
            )}
          </div>
          <div className="card" style={{padding:16}}>
          ...
          <div className="card" style={{padding:16}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {SEGS.map(s=>{const act=pSeg===s.id;return(
                <button key={s.id} onClick={()=>{setPSeg(s.id);setPDone(false);setPEmps([]);}} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:act?"rgba(99,102,241,.1)":"rgba(255,255,255,.02)",border:"2px solid "+(act?"#6366f1":"rgba(255,255,255,.06)"),borderRadius:12,cursor:"pointer",textAlign:"left"}}>
          ...
          <button onClick={iniciarScan} disabled={pScan} style={{width:"100%",background:pScan?"rgba(99,102,241,.1)":"linear-gradient(135deg,#6366f1,#818cf8)",border:"2px solid "+(pScan?"rgba(99,102,241,.2)":"transparent"),color:"#fff",borderRadius:14,padding:"18px",fontSize:18,fontWeight:800,cursor:pScan?"not-allowed":"pointer"}}>
          ...
          {pDone&&pEmps.length>0&&<>
            ...
            {!isPro&&<div style={{background:"rgba(99,102,241,.08)",border:"2px solid rgba(99,102,241,.25)",borderRadius:12,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
              <span style={{fontSize:24}}>🔒</span>
              <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:"#818cf8"}}>Versão Grátis — 5 empresas</div><div style={{fontSize:12,color:"#64748b"}}>PRO encontra até 15 por busca!</div></div>
              <button onClick={()=>setShowUpgrade(true)} style={{background:"linear-gradient(135deg,#6366f1,#818cf8)",border:"none",color:"#fff",borderRadius:9,padding:"9px 13px",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>Ver PRO</button>
            </div>}
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {pEmps.map((e,i)=>{const seg=SEGS.find(s=>s.id===e.segmento);return(
                <div key={e.id} className="card" style={{padding:14,border:"1px solid "+(e.enviado?"rgba(74,222,128,.2)":"rgba(255,255,255,.07)")}}>
                  <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}>
                    <div style={{width:42,height:42,borderRadius:10,background:"rgba(99,102,241,.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{seg?.emoji}</div>
                    <div style={{flex:1,minWidth:0}}>
                    ...
                    {e.enviado&&<span style={{background:"rgba(74,222,128,.1)",border:"1px solid rgba(74,222,128,.25)",color:"#4ade80",borderRadius:8,padding:"3px 8px",fontSize:11,fontWeight:700}}>✓ Enviado</span>}
                  </div>
          ...
          {!pScan&&!pDone&&<div style={{textAlign:"center",padding:"36px 0",color:"#1e3a5f"}}>
            ...
            <div style={{fontSize:17,fontWeight:700,color:"#2d4a6e",marginBottom:8}}>Pronto para buscar!</div>
            <div style={{fontSize:14,color:"#1e3a5f"}}>Selecione cidade, bairro e tipo de empresa.</div>
          </div>}
        </>}
          ...
          {ordens.map(o=>{const st=STATUS_OS[o.status];return(
            <div key={o.id} className="card" style={{padding:15}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:9}}>
                <div><div style={{fontFamily:"'Syne',sans-serif",fontSize:15,fontWeight:700}}>{o.clienteNome||"Sem cliente"}</div><div style={{fontSize:12,color:"#64748b",marginTop:2}}>OS #{o.numero} · {o.data}</div></div>
                <span className="badge" style={{color:st?.c,borderColor:st?.c+"40",background:st?.c+"10",fontSize:10}}>{st?.l}</span>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:"1px solid rgba(255,255,255,.06)",paddingTop:9}}>
                ...
                <div style={{display:"flex",gap:7}}>
                  <button className="btn btn-ghost" onClick={()=>gerarPDF(htmlDoc(o,emp),"OS-"+o.numero)} style={{fontSize:12,padding:"8px 11px"}}>🖨️ PDF</button>
        ...
        {tab==="crm"&&<>
          ...
          <div className="card" style={{padding:16}}>
            ...
            {clientes.length===0&&<div style={{textAlign:"center",padding:20,color:"#334155",fontSize:14}}>Nenhum cliente ainda.</div>}
            {clientes.map(c=>(
              <div key={c.id} style={{display:"flex",alignItems:"center",gap:11,padding:"9px 0",borderBottom:"1px solid rgba(255,255,255,.05)"}}>
                ...
                <div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.nome}</div><div style={{fontSize:12,color:"#475569"}}>{c.tel}</div></div>
              ...
              return(
                <div key={c.id} className="card" style={{padding:12,marginBottom:9,background:"rgba(255,255,255,.02)"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <div><div style={{fontSize:14,fontWeight:700}}>{c.nome||c.empresa}</div><div style={{fontSize:12,color:"#64748b"}}>{c.empresa}</div></div>
                    <span className="badge" style={{color:stC,borderColor:stC+"40",background:stC+"10",fontSize:10}}>{stL}</span>
                  </div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
                    {["novo","contato","proposta","fechado","perdido"].map(s=>{
                      const lbl={"novo":"Novo","contato":"Contato","proposta":"Proposta","fechado":"Fechado","perdido":"Perdido"}[s];
                      const sc={"novo":"#64748b","contato":"#fb923c","proposta":"#38bdf8","fechado":"#4ade80","perdido":"#ef4444"}[s];
                      return<button key={s} onClick={()=>setContatos(p=>p.map(x=>x.id===c.id?{...x,status:s}:x))} style={{padding:"4px 8px",borderRadius:7,border:"1px solid "+(c.status===s?sc:"rgba(255,255,255,.07)"),background:c.status===s?sc+"14":"transparent",color:c.status===s?sc:"#475569",fontSize:11,fontWeight:600,cursor:"pointer"}}>{lbl}</button>;
                    })}
                  </div>
                  <div style={{display:"flex",gap:7}}>
                    ...
                    <button onClick={()=>{setContatos(p=>p.filter(x=>x.id!==c.id));showWarn("Removido.");}} style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",color:"#ef4444",borderRadius:9,padding:"9px 10px",fontSize:13,fontWeight:700,cursor:"pointer"}}>🗑️</button>
                  </div>
                </div>
              );
            })}
        ...
        {tab==="fin"&&isPro&&<>
          ...
          <div className="card" style={{padding:16}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:14,fontWeight:700,marginBottom:10}}>Receitas — 6 meses</div>
            <BarChart data={g6m}/>
          </div>
          <div className="card" style={{padding:16}}>
            ...
            {financeiro.map(f=>(
              <div key={f.id} style={{display:"flex",alignItems:"center",gap:11,padding:"9px 0",borderBottom:"1px solid rgba(255,255,255,.05)"}}>
                ...
                <button onClick={()=>setFin(p=>p.filter(x=>x.id!==f.id))} style={{background:"none",border:"none",color:"#334155",fontSize:15,cursor:"pointer"}}>✕</button>
              </div>
            ))}
          </div>
        </>}
        ...
        {tab==="cfg"&&isPro&&<>
          ...
          <div className="card" style={{padding:16}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:11}}>
              {[{l:"Nome / Razão Social",f:"nome",p:"Ex: João Silva"},{l:"CNPJ / CPF",f:"cnpj",p:"00.000.000/0001-00"},{l:"CREA",f:"crea",p:"CREA-SP 123456"},{l:"Telefone",f:"tel",p:"(11) 99999-9999"},{l:"E-mail",f:"email",p:"joao@email.com"},{l:"Endereço",f:"endereco",p:"Rua..., nº, Cidade"},{l:"PIX",f:"pix",p:"CPF, CNPJ ou e-mail"}].map(fi=>(
          ...
          <div className="card" style={{padding:16}}>
            ...
            {servicos.map(s=>(
              <div key={s.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,.05)"}}>
                ...
                <button onClick={()=>setServicos(p=>p.filter(x=>x.id!==s.id))} style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",color:"#ef4444",borderRadius:7,width:28,height:28,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
              </div>
            ))}
          </div>
        </>}
      ...
      {modalOS&&<div className="mbg" onClick={()=>setModalOS(false)}>
        <div className="mdl" onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{display:"flex",gap:7}}>
              {[{v:"orcamento",l:"📋 Orçamento"},{v:"contrato",l:"📄 Contrato"}].map(t=><button key={t.v} onClick={()=>setFormOS(p=>({...p,tipo:t.v}))} style={{flex:1,padding:"10px",borderRadius:10,border:"2px solid "+(formOS.tipo===t.v?"#6366f1":"rgba(255,255,255,.07)"),background:formOS.tipo===t.v?"rgba(99,102,241,.1)":"rgba(255,255,255,.02)",color:formOS.tipo===t.v?"#818cf8":"#64748b",fontSize:13,fontWeight:700,cursor:"pointer"}}>{t.l}</button>)}
            </div>
            ...
            <div>
              ...
              {itensOS.length>0&&<div style={{background:"#0d1525",borderRadius:10,overflow:"hidden",marginBottom:9}}>
                {itensOS.map(it=><div key={it.id} style={{display:"flex",alignItems:"center",gap:7,padding:"8px 12px",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
                  ...
                  <button onClick={()=>setItensOS(p=>p.filter(x=>x.id!==it.id))} style={{background:"none",border:"none",color:"#ef4444",fontSize:14,cursor:"pointer"}}>✕</button>
                </div>)}
              </div>}
            </div>
            <div className="g2">
            ...
            {itensOS.length>0&&<div style={{background:"rgba(99,102,241,.07)",border:"1px solid rgba(99,102,241,.15)",borderRadius:10,padding:"12px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12,color:"#475569"}}>Subtotal</span><span style={{fontSize:13}}>{fmt(itensOS.reduce((a,b)=>a+b.q*b.v,0))}</span></div>
              <div style={{display:"flex",justifyContent:"space-between",borderTop:"1px solid rgba(255,255,255,.08)",paddingTop:6}}><span style={{fontSize:14,fontWeight:700}}>TOTAL</span><span style={{fontSize:19,fontWeight:800,color:"#4ade80"}}>{fmt(itensOS.reduce((a,b)=>a+b.q*b.v,0)-(Number(formOS.desconto)||0))}</span></div>
            </div>}
            ...
            <button className="btn btn-ghost" onClick={()=>{salvarOS();setTimeout(()=>{const o={...formOS,itens:itensOS,total:itensOS.reduce((a,b)=>a+b.q*b.v,0)-(Number(formOS.desconto)||0)};gerarPDF(htmlDoc(o,emp),"OS-"+(formOS.numero||"001"));},400);}} style={{width:"100%",fontSize:14,padding:"13px"}}>🖨️ Salvar e PDF</button>
            {editOSId&&<button className="btn btn-r" onClick={()=>{setOrdens(p=>p.filter(o=>o.id!==editOSId));setModalOS(false);showWarn("OS removida.");}} style={{width:"100%",fontSize:13,padding:"12px"}}>🗑️ Excluir</button>}
      ...
      {modalCli&&<div className="mbg" onClick={()=>setModalCli(false)}>
        <div className="mdl" onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:11}}>
            {[{l:"Nome",f:"nome",p:"Maria da Silva"},{l:"CPF/CNPJ",f:"cpfCnpj",p:"000.000.000-00"},{l:"Telefone",f:"tel",p:"(11) 99999-9999"},{l:"E-mail",f:"email",p:"email@exemplo.com"},{l:"Endereço",f:"endereco",p:"Rua, nº, bairro"}].map(fi=><div key={fi.f}><label className="lbl">{fi.l}</label><input className="inp" value={formCli[fi.f]||""} onChange={e=>setFormCli(p=>({...p,[fi.f]:e.target.value}))} placeholder={fi.p}/></div>)}
            <div><label className="lbl">Observações</label><textarea className="inp" rows={2} value={formCli.obs||""} onChange={e=>setFormCli(p=>({...p,obs:e.target.value}))} style={{resize:"vertical"}}/></div>
      ...
      {modalFin&&<div className="mbg" onClick={()=>setModalFin(false)}>
        <div className="mdl" onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{display:"flex",gap:8}}>
              {[{v:"receita",l:"📈 Receita",c:"#4ade80"},{v:"despesa",l:"📉 Despesa",c:"#ef4444"}].map(t=><button key={t.v} onClick={()=>setFormFin(p=>({...p,tipo:t.v}))} style={{flex:1,padding:"12px",borderRadius:10,border:"2px solid "+(formFin.tipo===t.v?t.c:"rgba(255,255,255,.07)"),background:formFin.tipo===t.v?t.c+"15":"rgba(255,255,255,.02)",color:formFin.tipo===t.v?t.c:"#64748b",fontSize:14,fontWeight:700,cursor:"pointer"}}>{t.l}</button>)}
            </div>
      ...
      {modalCon&&<div className="mbg" onClick={()=>setModalCon(false)}>
        <div className="mdl" onClick={e=>e.stopPropagation()}>
          <div className="hdl"/>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:17,fontWeight:800,marginBottom:16}}>📡 Prospecto</div>
          <div style={{display:"flex",flexDirection:"column",gap:11}}>
            {[{l:"Nome",f:"nome",p:"Ex: Carlos"},{l:"Empresa",f:"empresa",p:"Ex: Studio Arq"},{l:"Telefone",f:"tel",p:"(11) 99999-9999"},{l:"E-mail",f:"email",p:"email@exemplo.com"}].map(fi=><div key={fi.f}><label className="lbl">{fi.l}</label><input className="inp" value={formCon[fi.f]||""} onChange={e=>setFormCon(p=>({...p,[fi.f]:e.target.value}))} placeholder={fi.p}/></div>)}
